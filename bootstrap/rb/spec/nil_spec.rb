...
describe ::Acute::Nil do
  ...
  it "returns itself when being cloned" do
    @nil.perform(@nil, :msg => ::Acute::Message.new("clone")).should be @nil
  end

  it "does not evaluate its argument when receiving ifTrue" do
    @nil.perform(@nil, :msg => ::Acute::Message.new("ifTrue", [::Acute::Message.new("slotNames")])).should be @nil
  end

  it "evaluates the argument passed to ifFalse" do
    obj = ::Acute::Object.new
    @nil.register(:foo, obj)
    @nil.perform(@nil, :msg => ::Acute::Message.new("ifFalse", [::Acute::Message.new("foo")])).should be obj
  end
end